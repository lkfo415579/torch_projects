...
# BACKBONE_PATH = '../model/xlmr.large'
ROOT_PATH = f'..'
# # for colab
...
class ExcludeDuplicateSentencesTransform(NLPTransform):
    ...
    def __init__(self, always_apply=False, p=0.5):
        super(
            ExcludeDuplicateSentencesTransform,
            self).__init__(
            always_apply,
            p)
...
class SynthesicOpenSubtitlesTransform(NLPTransform):
    def __init__(self, always_apply=False, p=0.5):
        ...
        df = pd.read_csv(f'{ROOT_PATH}/input/op/open-subtitles-synthesic.csv', index_col='id')[['comment_text', 'toxic', 'lang']]
        df = df[~df['comment_text'].isna()]
        df['comment_text'] = df.parallel_apply(
            lambda x: clean_text(
                x['comment_text'], x['lang']), axis=1)
        ...
        if self.use_train_transforms:
            ...
            #     text, _ = synthesic_transforms(data=(text, label))['data']
                ...
                tokens, attention_mask = torch.tensor(
                    tokens), torch.tensor(attention_mask)
...
print("ATTEN_MASK:", attention_masks.shape)
# ### Class Balance
#
...
# df_val['comment_text'] = df_val.parallel_apply(
#     lambda x: clean_text(x['comment_text'], x['lang']), axis=1)

df_val['comment_text'] = df_val.apply(
    lambda x: clean_text(x['comment_text'], x['lang']), axis=1)
...
# df_test['comment_text'] = df_test.parallel_apply(
#     lambda x: clean_text(x['content'], x['lang']), axis=1)
df_test['comment_text'] = df_test.apply(
    lambda x: clean_text(x['content'], x['lang']), axis=1)
...
class RocAucMeter(object):
    ...
    def update(self, y_true, y_pred):
        ...
        self.score = sklearn.metrics.roc_auc_score(
            self.y_true, self.y_pred, labels=np.array([0, 1]))
...
class GPUFitter:
    ...
    def __init__(self, model, device, config):
        ...
        # self.optimizer = AdamW(optimizer_grouped_parameters, lr=config.lr * xm.xrt_world_size())
        self.optimizer = AdamW(optimizer_grouped_parameters, lr=config.lr * 1)
    ...
    def run_tuning_and_inference(self, test_loader, validation_tune_loader):
        for e in range(2):
            self.optimizer.param_groups[0]['lr'] = self.config.lr * 1 / (e + 1)
            # para_loader = pl.ParallelLoader(validation_tune_loader, [self.device])
            # losses, final_scores = self.train_one_epoch(para_loader.per_device_loader(self.device))
    ...
    def train_one_epoch(self, train_loader):
        ...
        final_scores = RocAucMeter()
        t = time.time()
        for step, (targets, inputs, attention_masks) in enumerate(
                train_loader):
            if self.config.verbose:
                if step % self.config.verbose_step == 0:
                    self.log(
    ...
    def run_inference(self, test_loader):
        ...
        result = pd.DataFrame(result)
        node_count = len(glob('node_submissions/*.csv'))
        result.to_csv(f'node_submissions/submission_{node_count}_{datetime.utcnow().microsecond}_{random.random()}.csv',
                      index=False)
...
def _mp_fn(rank, flags):
    ...
    para_model = nn.DataParallel(net).to(device)
    # net.to(device)
    ...
    fitter = GPUFitter(model=para_model, device=device, config=TrainGlobalConfig)
