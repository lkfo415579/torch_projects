...
def enhance_structure_dict(structure_dict):
    ...
    for molecule_name in structure_dict:
        ...
        # print("# Revo: pybel dir:", dir(mol.atoms[0]))
...
def unknown2_0(embeddings, iter, data, name):
    for t in range(iter):
        keys = set(data["type_" + str(t)])
        for k in keys:
            if k not in embeddings[(name, t)]:
                # there is not seeing embedding
                embeddings[(name, t)][k] = 0
    return embeddings
...
def add_embedding(atoms,bonds,triplets,quadruplets,embeddings=None):
    ...
    # for test data, by REVO
    if embeddings:
        embeddings = unknown2_0(embeddings, 3, bonds, 'bond')
        embeddings = unknown2_0(embeddings, 2, triplets, 'triplet')
        embeddings = unknown2_0(embeddings, 1, quadruplets, 'quadruplet')
